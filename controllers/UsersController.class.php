...
class UsersController extends BaseSQL{
	...
	public function changePWDAction(): void{

		$v = new View("changePWD", "front");

	}

	public function forgetPasswordToChangeAction(): void{
        ...
        if ($_POST['pwd1'] != $_POST['pwd2']){
            ...
            $user = new Users();
            $user = $user->getOneBy(['accesstoken'=>$_GET['accesstoken']],false);
            $pwd = password_hash($_POST['pwd1'], PASSWORD_DEFAULT);
            $query = $this->pdo->prepare("Update Users set pwd=:pwd WHERE id = :id");
            $query->execute(["pwd"=>$pwd,"id"=>$user['id']]);
            $_SESSION['success'] = "Le nouveau mot de passe est enregistré!";
        }
    ...
    public function forgetPasswordSendAction(): void{

        $user = new Users();
        $user = $user->getOneBy(['email'=>$_POST['email']],false);

        if (!empty($user)){
            $url = "http://".$_SERVER['HTTP_HOST']."/changePWD?accesstoken=".$user['accesstoken'];

            $content = "Veuillez cliquer le lien ci-dessous pour changer le mot de passe de votre compte ".TITLE." <br> ".$url;

            $mail = new Mail($user["lastname"],$user["email"],'Change MDP du compte de '.TITLE,$content);
            $mail->sendMail();

            $_SESSION["success"] = "Un mail a été envoyé, veuillez vérifier votre boîte aux lettres !";
        }else{
            $_SESSION["success"] = "On n'a pas trouvé votre mail!!";
        }
